...
final class EntityDto
{
    ...
    private $instance;
    private $warningKeyName;
    private mixed $warningKeyValue = null;
    ...
    public function __construct(string $entityFqcn, ClassMetadata $entityMetadata, ?string $entityPermission = null, /* ?object */ $entityInstance = null)
    {
        ...
        $this->warningKeyName = $this->metadata->getIdentifierFieldNames()[0];
    ...
    public function toString(): string
    {
        ...
        return sprintf('%s #%s', $this->getName(), substr($this->getwarningKeyValueAsString(), 0, 16));
    }
    ...
    public function getwarningKeyName(): ?string
    {
        return $this->warningKeyName;
    }

    public function getwarningKeyValue(): mixed
    {
        if (null === $this->instance) {
            return null;
        }

        if (null !== $this->warningKeyValue) {
            return $this->warningKeyValue;
        }
        ...
        $warningKeyValue = $propertyAccessor->getValue($this->instance, $this->warningKeyName);

        return $this->warningKeyValue = $warningKeyValue;
    }

    public function getwarningKeyValueAsString(): string
    {
        return (string) $this->getwarningKeyValue();
    }
